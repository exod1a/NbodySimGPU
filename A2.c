...
"	float dirvec[DIM];\n"\
"	for (int j = 0; j < DIM; j++)\n"\
"		dirvec[j] = r[j] - r[3*(idx+1) + j];\n"\
"	for (int j = 0; j < DIM; j++)\n"\
"	{\n"\
"		v[3*(idx+1) + j] += m[0] / (pow(pow(dirvec[0], (float)2.) + pow(dirvec[1], (float)2.) + pow(dirvec[2], (float)2), (float)3./2.)) * dirvec[j] * dt;\n"\
"		v[j] -= m[idx+1] / (pow(pow(dirvec[0], (float)2.) + pow(dirvec[1], (float)2.) + pow(dirvec[2], (float)2.), (float)3./2.)) * dirvec[j] * dt;\n"\
"	}\n"\
"}\n";
